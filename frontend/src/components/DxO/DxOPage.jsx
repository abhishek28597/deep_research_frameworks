...
import DxOAgents from './DxOAgents';
import DxOChatDisplay from './DxOChatDisplay';

export default function DxOPage({ selectedConversationId = null }) {
  ...
  const handleSubmit = async (query) => {
    try {
      ...
      if (!convId) {
        const conversation = await createConversation('DxO');
  ...
  return (
    <div className="max-w-6xl mx-auto min-h-screen">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-white">DxO</h1>
        {conversationId && (
          <button
            onClick={() => {
